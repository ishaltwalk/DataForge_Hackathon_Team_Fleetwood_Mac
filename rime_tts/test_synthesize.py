...
def test_baseline():
    ...
    path = synthesize("Welcome to Rime labs.", speaker="falcon", output_path="test_baseline.wav")
    check_wav(path)


def test_rpa_whole_string():
    print("\n[Test 2] Whole-string RPA — known example from Rime docs")
    path = synthesize("Welcome to labs.", speaker="falcon", rpa="r1Ym", output_path="test_rpa.wav")
    check_wav(path)
...
def test_rpa_mixed_sentence():
    ...
    path = synthesize(custom_text, speaker="falcon", rpa=None, output_path="test_mixed.wav")
...
def test_edge_cases():
    ...
    for filename, text in cases.items():
        path = synthesize(text, speaker="falcon", output_path=filename)
        check_wav(path)
